...
def find_images(data_dir, recursive=False):
    imgs = []
    for ext in IMAGE_EXTENSIONS:
        pattern = os.path.join(data_dir, "**", ext) if recursive else os.path.join(data_dir, ext)
        imgs += glob(pattern, recursive=recursive)
    imgs.sort()
    return imgs
...
class Datasets(Dataset):
    def __init__(self, data_dir):
        ...
        self.imgs = find_images(data_dir, recursive=True)
        if len(self.imgs) == 0:
            raise ValueError(f"No images found in test_data_dir: {data_dir}")
...
class Datasets_train(Dataset):
    def __init__(self, data_dir, img_size):
        ...
        self.imgs = find_images(data_dir, recursive=True)
        if len(self.imgs) == 0:
            raise ValueError(f"No images found in train_data_dir: {data_dir}")
