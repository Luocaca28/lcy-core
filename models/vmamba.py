...
def cross_selective_scan(
    ...
    B, D_in, H, W = x.shape
    # print(D)
    D_total, N = A_logs.shape
    K_weight, D, R = dt_projs_weight.shape
    L = H * W
    # print(R,N,N)
    if nrows < 1:
        if D % 4 == 0:
    ...
    indices = None
    if use_defscan:
        assert DS is not None and DR is not None
        deform_tokens, indices = DS(x)
        assert deform_tokens.shape == (B, L, D_in), (
            f"deform tokens shape mismatch: got {deform_tokens.shape}, "
            f"expected {(B, L, D_in)}"
        )
        assert indices.shape == (B, L), (
            f"indices shape mismatch: got {indices.shape}, expected {(B, L)}"
        )
        deform_tokens = deform_tokens.permute(0, 2, 1).unsqueeze(1)
        xs = torch.cat([xs, deform_tokens], dim=1)

    K_runtime = xs.shape[1]
    assert K_runtime == K_weight, (
        f"K mismatch: xs K={K_runtime}, weight K={K_weight}"
    )
    assert x_proj_weight.shape[0] == K_runtime, (
        f"x_proj K mismatch: {x_proj_weight.shape[0]} vs {K_runtime}"
    )
    assert D_total in (K_weight * D, D), (
        f"A_logs shape mismatch: A dim={D_total}, K*D={K_weight * D}, D={D}"
    )
    ...
    if x_proj_bias is not None:
        ...
        x_dbl = x_dbl + x_proj_bias.view(1, K_runtime, -1, 1)
...
class DefScanGate(nn.Module):
    def __init__(self, dim, hidden=64, init_bias=-1.5):
        super().__init__()
    ...
    def forward(self, x, snr):
        ...
        snr = snr.view(B, 1) / 20.0
        gate = torch.sigmoid(self.net(torch.cat([global_feat, snr], dim=1)))
...
class SS2D(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        base_k = scan_number if forward_type not in ["share_ssm"] else 1
        self.K = base_k + 1 if self.use_defscan else base_k  # 扫描大小这里也要改
        self.K2 = self.K if forward_type not in ["share_a"] else 1
        self.DS = (
            DeformableLayer(index=stage_index, embed_dim=d_inner, debug=False)
            if self.use_defscan
            else None
        )
    ...
    def forward_corev2(self, x: torch.Tensor, nrows=-1, SNR=10, channel_first=False):
        ...
        use_defscan = getattr(self, "use_defscan", False)
